...
extern uint16_t wav_buffer[WAV_BUFFER_SIZE];
...
public:
    WAVWriter(uint8_t* buffer, size_t buffer_size, int sample_rate, int bit_depth);
    bool write(uint8_t* samples, size_t count);
    void finish();
};